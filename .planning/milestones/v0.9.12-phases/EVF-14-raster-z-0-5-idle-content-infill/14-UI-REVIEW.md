...
1. **WARNING — UI-SPEC §10 width-budget table drifts from fixture bytes (Consistency w/ design contract).**
   ...
   - **Resolution:** Closed in Phase 18 — UI-SPEC §10 re-derived from `idle-infill-layer.ts` runtime literals (Option (a), the lower-risk doc-only path). Label-separator now documented as 52 cells (raster) / 40 cells (glyph); stats strip 54 cells (raster) / 51 cells (glyph). See 18-PLAN.md Task 3.
...
2. **WARNING — UI-SPEC §2 spacing token table cites wrong divider column (Spacing / Alignment).**
   ...
   - **Resolution:** Closed in Phase 18 — UI-SPEC §2 col-anchors corrected to col 67 (right-stop) and 64 cells (content-width); central-divider note added. Frame-corner enumeration updated from {0, 71, 95} to {0, 68, 95}. See 18-PLAN.md Task 3.
...
3. **WARNING — Locale leak in State C IT fixture (Copy / Tone).**
   ...
   - **Resolution:** Closed in Phase 18 — IT fixture rows 1+17 fixed + 4 additional IT-locale leaks (rows 5/7/9/12: PF/CA/Az./Slot) discovered via the new Z05-INV-02b-triade test and fixed in the same commit (deviation Rule 2 — auto-add missing critical functionality, broader scope than plan called out). Z05-INV-02b-triade extension (A_it ↔ B_it ↔ C_it byte-identity for cols 69..95 rows 3..20, with surgical exemption for row 20 cols 89..93 `[GLY]` glyph-mode marker per UI-SPEC §6.3) prevents regression. See 18-PLAN.md Task 1+2.
