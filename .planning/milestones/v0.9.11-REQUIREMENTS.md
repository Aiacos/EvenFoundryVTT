# Requirements Archive: v0.9.11 MVP

**Archived:** 2026-05-17
**Status:** SHIPPED

For current requirements, see `.planning/REQUIREMENTS.md` (next milestone). REQUIREMENTS.md was `git rm`'d during milestone close per the new-milestone convention; a fresh requirements file will be created for v0.9.12.

## Final outcomes

- **48/48 v1 REQ-IDs software-complete** per traceability table (canonical source). All MVP-scope requirements landed across Phases 0–10.
- **Checkboxes flipped at archive**: 21 active `[ ]` + 1 `[~]` MIDIQ-01 (code-complete) → `[x]`. Total: 43 listed items in the v1 checklist; **5 items appear only in the traceability table** (minor pre-existing checklist/traceability skew, did not block close).
- **MIDIQ-01 special note**: code-complete in Phase 0 Plan 02; evidence file (`docs/perf/phase-0/midiqol-config-probe-{ISO8601}.json`) deferred to operational probe run with Foundry test world `phase-0-midiqol-test`. Carry-forward as part of ADR-0005 Branch A.
- **Hardware-pending verification carry**: 35 success criteria across Phases 4a/4b/5/6/7/8/9/10/12/13 marked `human_needed`. Closure via `pnpm --filter @evf/validation-harness validate:all` once Even Hub access + G2 + R1 + consenting DM available.
- **V2 OPZIONALE shipped early**: VOICE-01..05 (Phase 11-12) + ACT-04 partial (Phase 13) + STRETCH-06 portrait flag-gated (Phase 13). VOICE-* + ACT-04 + STRETCH-* not counted in v1 coverage — they remain marked as **v2** but their implementations are present in the codebase as flag-gated optional surface.
...
- [x] **DISP-01**: HUD persistente con status PG (HP/AC/azioni/slot/condizioni) sempre visibile in corner card
- [x] **DISP-02**: Layout layered (z=0 mappa, z=1 status HUD, z=2 overlay panel) con esattamente 1 capture container
- [x] **DISP-03**: Layout integrity garantita per tutti gli stati (INV-1, snapshot test §7.14.4 ck 11–15)
- [x] **DEATH-01**: Death saves status HUD durante HP=0 (3-strike tracker visivo, momento viscerale 5e). *Adversarial gap Vector A.*
- [x] **TOAST-01**: Toast queue/stack design (max 2 visibili FIFO 3s, overflow squash to "+N more"). *Adversarial gap Vector C — Fireball + 8 saves overflows naive impl.*
- [x] **BOOT-01**: Boot error states ortogonali (handshake failed / version mismatch / no character / bridge unreachable / token expired). *Adversarial gap Vector C — spec mockup happy-path only.*

### Map Rendering

- [x] **MAP-01**: Raster pipeline 4-bit greyscale dithered (4 image container 2×2 = 400×200 px effective) — DEFAULT MVP (subordinate a Phase 0 GO/NO-GO)
- [x] **MAP-02**: Glyph mode fallback (text grid 96×24 char) — alternativa user-selectable runtime
- [x] **MAP-03**: 6-layer optimization stack (delta hash · sub-tile encoding · static caching · custom RLE · BLE 4.2+ DLE · adaptive frame rate)
- [x] **MAP-04**: 5 fps standard committed / 15 fps aspirational (subordinate a §10.0.3, §10.0.6, §10.0.7)
- [x] **MAP-05**: Mode toggle runtime via Quick Action `[M] Map ctrl` (raster ↔ glyph, hot-swappable)
...
- [x] **NAV-04**: Boot splash → handshake → main HUD flow con capability negotiation

### Foundry Integration

- [x] **FOUN-01**: Foundry module `evenfoundryvtt` espone read API (`getCharacterState`, `getCombatState`, `getSceneViewport`, `getEventLog`, `subscribeUpdates`)
- [x] **FOUN-02**: Bridge service Node.js (Fastify + ws + Docker Compose) come reverse-proxy CORS-friendly + auth bearer 24h
- [x] **FOUN-03**: Write path via `activity.use()` + MidiQOL workflow (GM veto power preserved, no nesting)
- [x] **FOUN-04**: `TokenLayer.setTargets()` v13 multi-target (singolare `Token`, no `Tokens`)
- [x] **MIDIQ-01**: MidiQOL config check al boot — verificare `autoFastForward` mode attivo. Senza, manual write stalla su chat-card buttons. Declare MidiQOL **required** in `module.json` `relationships.requires`. *Adversarial gap Vector B — Specs §12.B q.11–12 open.* **Code complete 2026-05-10** (Plan 02: probe + Foundry mini-module + harness, commits 15e9922 + c1c82e5). `relationships.requires.midi-qol` declaration shape proven via probe `module.json`; Phase 2 production module inherits identically. **Evidence emission pending Plan 04 operational step** (researcher executes probe against Foundry test world `phase-0-midiqol-test` → commits resulting `docs/perf/phase-0/midiqol-config-probe-{ISO8601}.json`).

### i18n / Localization

- [x] **I18N-01**: Locale auto-detected da `game.i18n.lang` al boot (3-layer arch: Foundry SoT → Bridge → G2 runtime)
- [x] **I18N-02**: Runtime override via Quick Action `[N] Language` (device-local, non tocca world settings)
- [x] **I18N-03**: Catalogi forniti da Foundry (dnd5e + module `evenfoundryvtt`) — G2 ships no strings
- [x] **I18N-04**: Width-budget per chiave + fallback EN se eccede (INV-1 i18n stress, ck 14)
- [x] **I18N-05**: MVP target IT + EN canonical; best-effort DE / ES / FR / PT-BR

### Connection Bootstrap (Phone-Side)

- [x] **CONN-01**: Phone setup wizard nell'Even Realities App WebView (3-step: profile/URL → token → character). *Render phone-side, NOT G2.*
- [x] **CONN-02**: Bridge URL + auth token persistente phone-side (Tier 3 storage §11.5.5, sopravvive a kill/restart/reboot)
...
- [x] **CONN-04**: Auto-connect on G2 wear (Even Realities App detecta wear via SDK → riapre plugin)
