...
Last activity: 2026-06-05 -- Completed quick task 260605-dog: bridge now caches the latest character.delta per actorId and internalSnapshotFn serves it for evf.getCharacterSnapshot — so d0v's on-connect initial push actually fires in prod and GET /v1/character/:actorId stops returning actor_not_found (5th handler in the /internal/delta fanout, mirrors spell/entity caches). 462/462 bridge tests. Live sim verification next.
