...
Plan: —
Status: Awaiting next milestone
Last activity: 2026-06-11 - Completed quick task 260611-e71: modulo v0.1.15 rilasciato (frame_png 37× meno banda, captureFps 1-60, no scheduler cap)
...
| 2026-06-05 | `260605-dog-bridge-caches-the-latest-character-delta` | `900cd0c` | **Bridge character-snapshot cache (makes d0v's on-connect push actually fire in prod)** — d0v's `pushInitialCharacterDelta` no-opped in real prod because `internalSnapshotFn` returned null for `evf.getCharacterSnapshot` (verified live: `GET /v1/character/:actorId` → `actor_not_found`). Fix (bridge-only, mirrors SpellPackCache/EntityPackCache): (1) new `cache/character-snapshot-cache.ts` `CharacterSnapshotCache` — `Map<actorId, CharacterSnapshot>`, last-write-wins, no TTL (matches sibling caches), set/get/clear; (2) new `ws/character-snapshot-handler.ts` `handleCharacterSnapshotEnvelope(type,payload,cache)` — on `type==='character.delta'` validates `CharacterSnapshotSchema.safeParse` then caches by `snapshot.actorId` (character.ts:489); false on type mismatch / invalid (no throw), mirrors handleSpellPackEnvelope/handleEntityPackEnvelope; (3) wired as the 5th handler in the `/internal/delta` fan-out + `opts.characterSnapshotCache` in BuildServerOptions + `internalSnapshotFn` new `evf.getCharacterSnapshot` branch reading `args[0]=actorId` → `cache.get(actorId) ?? null` (keeps listCharacters + null default). Now a `character.delta` pushed to `/internal/delta` is BOTH fanned to WS sessions (unchanged) AND cached, so `GET /v1/character/:actorId` returns 200 and a fresh WS connect replays the snapshot via d0v's push. TDD: 8 unit + 3 integration (POST character.delta → GET /v1/character 200; seeded roster + cached snapshot → fresh WS connect receives initial character.delta — ties d0v+dog end-to-end). 462/462 bridge tests, tsc + biome clean, socketlib count 17 (no foundry-module touched), no new deps. Branch `feat/real-foundry-pairing`. |
| 2026-06-05 | `260605-e9t-g2-app-boot-engine-wsevents-bus-attach-a` | `0ca5dc5` | **g2-app wsEvents last-value replay (makes the on-connect HUD render actually land) — END-TO-END SIM VERIFIED ✓.** Root cause of the HUD showing placeholders on first connect despite d0v+dog: `createWsEventBus` (boot-engine-core.ts) attached its `message` listener only inside `subscribe()` and kept NO buffer, and the bus was created at step ~11 (after handshake); `StatusHudLayer` subscribes only at step 12 (`lm.bundle` mount). The bridge's on-connect `character.delta` arrives right after the handshake response (steps 7-11) → dropped. Fix (g2-app only, TDD): refactored `createWsEventBus` to (1) ONE persistent `globalHandler` attached at bus creation (never removed), (2) `lastByChannel: Map<string,unknown>` caching last payload per channel, (3) `subscribe()` synchronously replays the cached value before registering for futures, (4) `setPerfProbe()` late-binds the PerfProbe after `handshake.session_id` exists, (5) **bus created at step 5a — after WS open, BEFORE performCapabilityHandshake** so the handler is live for the on-connect push (performCapabilityHandshake uses its own `{once:true}` listener so they coexist), (6) Phase 10 `seqTracker.observe()` + `perfProbe.mark('result_envelope')` hot-path preserved verbatim. WsEvents interface unchanged. 7 new unit tests (pre-subscribe replay · post-subscribe forward · per-channel isolation · last-value-only · seq+perf still fire · scoped unsubscribe · pre-handshake ordering). 1429/1429 g2-app tests, tsc + biome clean, no new deps. **LIVE SIM PROOF (d0v+dog+e9t together):** local bridge (my code) on :8911 dev-no-auth, roster + a seeded Artemis snapshot pushed to `/internal/delta`, g2-app vite repointed to :8911, EvenHub sim relaunched → on the FIRST connect (no post-connect delta) the glasses HUD rendered **Artemis · PF bar 55/88 · CA 18** (the seeded value) instead of placeholders. First time real character data renders on the glasses on connect end-to-end. Branch `feat/real-foundry-pairing`. |
| 2026-06-05 | `260605-etr-production-clean-the-g2-app-glasses-hud-` | `b867327` | **Production-clean HUD: header + footer now render real content (no more SDK "Text" placeholder) — LIVE SIM VERIFIED.** User reported "caratteri sparsi" on the glasses: root cause = the `header` (id4) and `footer` (id5) text containers were never populated post-boot → showed the EvenHub SDK default literal "Text". Fix (g2-app only, implements the EXISTING canonical Specs §7.4 mockup + frozen INV-1 fixture `glyph-scene.raster-idle-it.txt` — so NO INV-3/spec change, "implements-existing-mockup" case): new `engine/hud-chrome.ts` `writeHeaderChrome` (frame-top `MAP · <scene> · <mode>  ROUND <n> · <TURNO|TURN> x/y  ⌁ R1 <nn>%`, missing scalars → em-dash fallback) + `writeFooterChrome` (footer breadcrumb/hint `R1: scroll=pan tap=ping long=quick  modo: ▶RASTER (toggle GLYPH)  [scheda] [combat]`, IT/EN), wired at boot step 12a AFTER the bundle flush (the flush resets containers to SDK default; chrome has no owning layer so it must paint post-flush). Kept `long=quick` (NOT `qa=` — GEST-01/ADR-0012 gesture-vocab sweep is Phase 20). 13 new regression tests (HC-1..HC-7), 1442/1442 g2-app tests, tsc + biome clean, INV-1 zero fixture drift. Live sim: header `MAP · raster · TURNO … R1`, footer `R1: scroll=pan …` now render. **KNOWN REMAINING (sibling bug → next task):** `map-capture` (id7) + z05 idle-infill (id8-10) still show SDK "Text"/"T-.." because `idleInfill.draw()` is never called post-bundle (only `mapBase.draw()` at step 13, which no-ops without a scene); the boot comment assumes a post-bundle self-redraw that doesn't happen. Branch `feat/real-foundry-pairing`. |
| 2026-06-05 | `260605-f9s-g2-app-boot-call-idleinfill-draw-after-l` | `3b3d4a6` | **HUD idle render fix (kills the last SDK "Text" placeholders) — LIVE SIM VERIFIED, display now fully clean.** Sibling of etr: the post-bundle `rebuildPageContainer` flush reset containers to SDK "Text", and nothing repainted `map-capture` (id7) or the z05 idle-infill (ids 8-10) afterward — the boot step-12a comment falsely claimed idle infill "self-redraws". Fix (g2-app only, implements existing fixtures): (FIX1) extracted exported `finalizeIdleRender(idleInfill, mapBase)` in boot-engine-core.ts that awaits `idleInfill.draw()` then `mapBase.draw()` each under its own try/catch, called at step 13 (replaces the bare `mapBase.draw()`); corrected the misleading comment. (FIX2) `MapBaseLayer.draw()` resolves mode before the null-scene early-return and, in the raster + no-frame case, writes an empty-string `textContainerUpgrade('map-capture','')` to clear the SDK default (image tiles map-tile-0..3 sit over a blank capture container per the raster-idle fixture); glyph + no-scene stays a true no-op (glyph map area is owned by glyph-renderer). 6 new tests (MBL-IDLE-01..03, F9S-BOOT-01..03), 1448/1448 g2-app tests, tsc + biome clean, INV-1 zero fixture drift, no fixture edits. **LIVE SIM (full clean HUD):** header `MAP · raster · TURNO … R1`, map area blank, z05 `─── 0.5 idle infill ───` + `raster — · BLE — fps · [Q] Quick`, footer `R1: scroll=pan …`, status-hud `Artemis · PF 55/88 · CA 18` — zero SDK "Text" anywhere. The `0.5 idle infill` label is spec-ratified content (Phase 14, design-locked). Branch `feat/real-foundry-pairing`. |
| 2026-06-05 | `260605-flv-wire-character-selection-end-to-end-so-t` | `dfcaa43` | **Character selection wired end-to-end — LIVE SIM VERIFIED (HUD renders the CHOSEN PC).** Before: wizard step3 saved `characterId` to Tier3 but it never reached the engine/bridge; bridge always pushed `roster.characters[0]`. Fix (3 tasks, TDD, cross-package shared-protocol+bridge+g2-app): (1) `HandshakeClientSchema` gains optional `actorId` (additive); bridge `Session` gains `selectedActorId?`, `createSession(token,locale,caps,selectedActorId?)`, `handleHandshake` persists `client.actorId` on first-connect + reconnect-not-found (reconnect-found preserves existing); (2) `pushInitialCharacterDelta` selects `selectedActorId ?? roster.characters[0]?.actorId` (server.ts passes `session.selectedActorId`); `emitDelta` only delivers a `character.delta` to a session when (three-present AND-guard) `type==='character.delta'` && session has `selectedActorId` && payload has string `actorId` && they match — else falls through to today's broadcast (prevents cross-player character leakage without breaking existing emitDelta tests); (3) g2-app `BootEngineOpts.characterId` → `performCapabilityHandshake` `actorId` (both initial + reconnect); `launchApp` resolves `?actor=<id>` URL override > Tier3 `session.characterId` > undefined, threaded into the no-auth dev bootEngine call + reusable resolver (Branch B wizard path untouched). Wire field = `actorId`; Tier3 field stays `characterId` (same value). dog cache makes any pushed actorId serveable. 3109/3109 workspace tests (+58), typecheck + lint:ci clean, socketlib 17 (no foundry-module), no new deps. **LIVE SIM:** loaded `?actor=6KWxQXAiJgz4zKlS` (Dante) + seeded Dante snapshot → HUD rendered **Dante Lanzu… · PF 41/63** instead of Artemis. Branch `feat/real-foundry-pairing`. |
| 2026-06-05 | `260605-g54-prepare-real-release-changesets-for-this` | `a4a7180` | **Real release prep: changesets + self-hosting guide.** (1) 3 minor changesets for the session's work — `.changeset/g54-bridge-on-connect-character.md` (@evf/bridge: d0v on-connect push + dog snapshot cache + flv selectedActorId/targeting), `g54-g2app-render-and-selection.md` (@evf/g2-app: e9t replay + etr/f9s clean HUD + flv selection), `g54-shared-protocol-handshake-actorid.md` (@evf/shared-protocol: optional handshake actorId). `changeset:status` → bridge/g2-app/shared-protocol minor; NOT versioned (CI Release PR owns `changeset version`). (2) g2-app build verified (`corepack pnpm --filter @evf/g2-app build` → dist, 244 modules); app.json whitelist left at safe `REPLACE-WITH-YOUR-ORIGIN.example` placeholders (per-user origins via `deploy/sync-app-whitelist.mjs`); no `.ehpk` committed. (3) new `docs/self-hosting.md` (283 lines, 6 steps: deploy/.env → docker compose + .https.yml Caddy → build+sync-whitelist+pack g2-app → install on glasses via Even Hub dev-QR personal / manual portal public (NO CLI publish) → foundry-module install + Bridge Config dialog + pairing → wizard character pick → HUD) + README "Self-Hosting" pointer. INV-3 additive-only (Specs §3.3/§3.7/§11.5.3 already cover the model; no Specs/showcase edit). Branch `feat/real-foundry-pairing`. **Even Hub portal upload remains a MANUAL user step (review-gated); for physical-glasses testing use dev-mode QR pointing at the user's own bridge.** |
| 2026-06-05 | `260605-hd5-one-command-simulator-dev-test-harness-c` | `e87981b` | **One-command simulator dev/test harness — `pnpm sim` (effortless sim testing).** Formalizes the proven manual flow into `scripts/sim.sh` (start/stop/seed/shot) + `pnpm sim` alias. `pnpm sim start [--actor <id>]` boots all services: no-auth bridge (`EVF_DEV_NO_AUTH=true EVF_INTERNAL_SECRET=dev-secret PORT=8911`, tsx), seeds committed schema-valid fixtures (`scripts/sim-fixtures/`: roster.json + character-{artemis,dante,karius,shin}.json — actorIds Artemis E14Tfh9Ba07cpPyM hp55/88 ac18 · Dante 6KWxQXAiJgz4zKlS 41/63 16 · Karius 4GXG7ufxylS4H1Pk 70/70 20 · Shin VoNfASW4hQ4dG4cv 12/48 14) via `/internal/delta`, starts vite no-auth pointed at the bridge, launches the EvenHub sim (headless xvfb+GTK env auto when no $DISPLAY, direct otherwise), prints a READY cheatsheet (services + actorIds + screenshot/console/input API). `pnpm sim stop` = `fuser -k` teardown (NOT pkill-f). `pnpm sim shot [path]` saves the glasses PNG. 4 footguns encoded (GTK env, fuser-k, in-memory re-seed, ?actor= on sim URL). All 4 fixtures verified schema-valid (bridge safeParse → GET /v1/character/:id = 200). `docs/simulator-testing.md` (290 lines) + README pointer + `.sim-run/` gitignored. **Orchestrator re-verified: `pnpm sim start` → READY cheatsheet → `pnpm sim shot` → clean HUD (Artemis).** Branch `feat/real-foundry-pairing`. **NOTE — KNOWN ISSUE surfaced this session: "scritte troppo grandi" = the G2 LVGL font has a FIXED 27px line height (~10 rows on 288px, no font control per SDK), but the HUD layout (container-registry geometry h12/h24 + Specs INV-1 mockups + status-hud 21-row renderer) was designed for a ~12px/24-row grid → text overflows/overlaps/clips and looks ~2.25× too dense. Real fix = a layout redesign for the 27px grid (container-registry + status-hud-renderer + Specs §7 INV-1 mockups + fixtures + showcase) — spec-level, needs design decisions on what fits in ~9 rows. NOT yet done; flagged for a dedicated effort.** |
| 2026-06-05 | `260605-j0t-redesign-the-g2-hud-for-the-real-27px-fo` | `d05574c` | **HUD text-layout redesign for the real 27px grid — DONE + live-sim clean (but see PIVOT below).** Fixed "scritte troppo grandi" for the text-container HUD: default glasses view = full-width character-status sheet (was the narrow 168px column), raster map deferred to a future gesture mode. `status-hud-renderer.ts` rewritten to an **8-row full-width sheet** (name+Lv+class · divider · HP bar+cur/max+CA+VEL · Turno/Round/your-turn · Cond · divider · Slot · TS morte), pixel-width-budgeted via `@evenrealities/pretext` getTextWidth (≤576px, variable-width font — NOT char count). container-registry → full-width 27px geometry; boot default-view = 3 status containers (header id4 + status-body id6 + footer id5); MapBaseLayer/IdleInfill preserved-but-not-drawn. Data gaps (class/speed/turn/round NOT in CharacterSnapshot) render as `—` with TODO(HUD-27PX). 5 commits (j0t-01 renderer TDD · 02 geometry+boot · 03 INV-1 fixtures+INV-3 atomic Specs v0.9.14 · **04 fix: boot crash — default page declared the overlapping map-capture id7+z05; now status-view-only** · **05 fix: `_flushPage` was re-declaring the full registry → "Text" ghosting/overlap, now uses the 3-container status schema; SHEET_ROWS 9→8 to fit id6 h234 + drop the footer-duplicate R1 hint line; Specs v0.9.15**). 1435/1435 g2-app tests, tsc + lint clean. **Live-sim verified: clean full-width sheet, no overlap, real data (Artemis Lv10 · PF bar 55/88 · CA 18 · TS morte).** ⚠️ **PIVOT (user, 2026-06-05): switch the HUD to IMAGE/raster rendering ("tutto a immagini", 5fps ok) — render the HUD via OffscreenCanvas → 4-bit dither → 4 image tiles (288×144 ×4 = 576×288 full screen) for full font/layout control, bypassing the SDK 27px text-container limit. The j0t renderer's CONTENT logic (what to show) is reusable; the OUTPUT target changes from text containers to a canvas image. Next effort. NOTE on "generate with Python?": NO — runtime renders in the phone WebView (JS/OffscreenCanvas) or optionally the Node bridge (sharp); the raster pipeline (image-q/upng/xxhash) is already JS. Python would fragment the TS/Node stack; not recommended.** Branch `feat/real-foundry-pairing`. |
| 2026-06-11 | `260611-clr-opzione-impostazioni-g2-app-per-attivare` | `86e2c12` | **Toggle dither on-glasses.** Nuova voce `[D] Dither` nel Quick Action menu (pattern `[F] FPS`): helper `hud/dither-mode.ts` persistito su Even Hub kv (`view.hud.dither`), flag `dither` threaded nel tile-build sync (`hud-raster-frame.ts`) + Worker twin (`hud-tile.worker.ts`) + worker client + delta-driver letto a inizio ciclo (ON = Bayer 4×4 default, OFF = quantizzazione diretta a 16 livelli). 5 commit TDD; test + tsc + biome clean; changeset patch g2-app. |
| 2026-06-11 | `260611-dg5-trailing-edge-throttle-re-arm-in-huddelt` | `6682d09` | **HUD delta-driver: throttle trailing-edge re-arm → 29-30 fps stabili (bench sim).** Root cause del plateau 16-20 fps: `_schedule()` era leading-edge — eventi durante la finestra pending scartati, periodo reale = interval+cycleTime (~48-60ms). Fix: `_pendingAgain` + `_cycleInFlight`, re-arm in `finally` a `max(0, 33ms − elapsed)` → periodo = `max(interval, cycleTime)`. Bench live (sim headless, frame_pixels 400×200): input 30fps → **29-30 fps** delivered (era 16-17); input 60fps → 30 fps stabile (cap 33ms). Test DL-09..13 (cadenza, no-loss, no-overlap, idle, stop-during-pending); 1678 test verdi. |
| 2026-06-11 | `260611-e71-modulo-v0-1-15-frame-png-captureinterval` | `3ff35fc` | **Modulo v0.1.15 RILASCIATO — stream mappa PNG + captureFps.** Nuovo wire `frame_png` (PNG greyscale lossless via upng-js `encode(...,forbidPlte=true)`, ~11KB vs ~417KB RGBA b64 = 37× meno banda); skip frame identici (FNV-1a su luma); hook throttle leading+trailing 200ms (pan continuo ~5fps, prima 0); world setting **`captureFps` 1–60 default 4** (review pass utente: era `captureIntervalMs` con tick-gate 100ms = cap 10fps → loop setTimeout auto-schedulante, NESSUN cap scheduler); g2-app decode `frame_png` con back-compat `frame_pixels`; bridge pass-through testato. **Fix review (no-GSD)**: Buffer-pool bug (`Buffer.from(b64).buffer` = pool con byteOffset≠0 → "not a PNG file") in scene-input PRODUZIONE + 4 siti test. Live-verificato nel sim: frame_png 30fps input → 26-29fps delivered; frame_pixels legacy 10fps OK. 3439 test verdi, tsc+biome 0. Release: `gh release v0.1.15` (zip+manifest). Plan-checker PASSED; verifica = review manuale su richiesta utente. |
...
| 2026-06-18 | `260618-d72-headless-player-view-workflow-fixes` | `38790fb`,`b8cf387`,`d297f6c`,`edcb8f1` | **4 ADR-0015 §C player-view workflow bugfixes (atomic).** Root symptom "streaming headless non torna alla vista party". BUG 1 (`38790fb`): streaming joined an arbitrary first `/join` user → add `EVF_PLAYER_VIEW_STREAM_USER`, select-by-label in `tryFoundryJoin` (fallback first-option). BUG 3 (`b8cf387`): Foundry auto-entry could bypass `/join` → assert `game.user.name` matches the requested user post-`canvas.ready`, mismatch throws → orchestrator `error`; exported `requestedUserFor()`. BUG 4 (`d297f6c`): roster had no heartbeat → stream leader re-publishes `r1.characters.available` every 10s (mirrors settings heartbeat) so the bridge cache survives a restart; module v0.1.33 → **v0.1.34**. BUG 5 (`edcb8f1`): roster not replayed to late WS subscribers → `sendInitialToSession(R1_CHARACTERS_AVAILABLE_TYPE)` on subscribe (mirrors settings.display replay). All gates green: lint:ci 0, typecheck 0, bridge 593, foundry-module 671 (+known teardown flake). Live re-test PENDING: bridge image rebuild + set `EVF_PLAYER_VIEW_STREAM_USER` (BUG 1/3/5); module redeploy v0.1.34 to Foundry (BUG 4). BUG 2 (forced-leader suppression) correct-by-design, no change. |
...
| 260610-evs | Pre-dither contrast normalization for the glasses map stream as a user-settable Foundry option: `extractCurrentFrame` gains `normalize:'off'\|'auto'` (Rec.709 luma histogram, p2/p98 levels-stretch on content pre-letterbox, skip-guards range>=220 / range<8); `getNormalize` consulted per capture (live toggle, no re-register); new CLIENT-scope config setting `mapContrastNormalize` (default true, i18n IT/EN). Live-sim proof on real Foundry 3440x1440 frame: off vs auto (`/tmp/evf-shots/98-norm-compare.png`). 571/571 foundry-module tests. | 2026-06-10 | a5a7406 | Verified (live sim) | [260610-evs-contrast-normalization-setting-for-glass](./quick/260610-evs-contrast-normalization-setting-for-glass/) |
| 260610-fff | Exempt POST /internal/delta from the bridge global rate limit (route-level `config.rateLimit:false`; auth unchanged, all other routes keep 100/min): unblocks the v0.1.9 ~1Hz map stream — evidence 1102 prod 429s during the 2026-06-09 game session before the stream even existed. Tests FFF-RL-01..03 (150-flood zero 429, non-exempt route still 429s, wrong secret still 401). 480/480 bridge tests. | 2026-06-10 | ad77a94 | Verified (tests) | [260610-fff-exempt-internal-delta-route-from-bridge-](./quick/260610-fff-exempt-internal-delta-route-from-bridge-/) |
| 260610-fw7 | Fix canvas-extractor frame corruption found on the REAL Forge client: `extract.pixels(canvas.stage)` returned a world-bounds-sized buffer interpreted with renderer dims (stride mismatch → striped garbage). Switched to VIEWPORT capture (`pixels()` no-arg = what the player sees, fog/zoom applied, no whole-world re-render at 1Hz) + fail-loud byte-length guard with integer-resolution inference (k>=2) else warn+skip. Tests CE-VP-1..3. Implements the Phase 27 viewport-extraction decision. | 2026-06-10 | fb95f03 | Verified (tests) | [260610-fw7-fix-canvas-extractor-stage-vs-viewport-s](./quick/260610-fw7-fix-canvas-extractor-stage-vs-viewport-s/) |
| 260610-lx5 | Render-to-texture viewport capture in canvas-extractor (final live-chain fix): v0.1.10's no-arg `pixels()` read the WebGL main framebuffer, valid ONLY during the render pass (preserveDrawingBuffer:false) → all-zero frames whenever the Foundry canvas idled (live-verified maxG=0 at ~1Hz). Primary path now renders the stage into a viewport-sized PIXI RenderTexture and extracts from it (deterministic, fog/zoom included), rt.destroy(true) in finally; no-arg fallback kept for fixtures. Tests CE-VP-4..7. | 2026-06-10 | 9eaed5a | Verified (tests) | [260610-lx5-render-to-texture-viewport-capture-in-ca](./quick/260610-lx5-render-to-texture-viewport-capture-in-ca/) |
| 260610-n8h | Floor fractional renderer.screen dims in the RT capture path: at DPR 1.333 Foundry reports 2348.25x824.25, PIXI floors the texture, the fractional expected-length check rejected EVERY frame. Proven via live console experiment on the user's Forge client (A_rt maxG=255 = RT capture works; PIXI 7.4.3). Test CE-VP-8. | 2026-06-10 | e8233b8 | Verified (tests) | [260610-n8h-floor-fractional-viewport-dims-in-rt-cap](./quick/260610-n8h-floor-fractional-viewport-dims-in-rt-cap/) |
...
Last session: 2026-06-08 (evening) — live-sim smoke test + debug of v0.10.0 canvas raster HUD
Stopped at: v0.10.0 milestone closed/archived. Ran first live-sim smoke test of the new canvas raster HUD → found a doubled status header → `/gsd-debug` RESOLVED it. The "blank body" and "WS drop" were ruled NON-bugs (blank body = expected Phase 20: full sheet is a gesture-opened z=2 overlay; WS persistent, delta pipeline silent on success). Real bug = status header overlap, fixed via CHROME-01 (canvas-mode guard in boot-engine-core.ts) + FIX-DD-01 (measureText positioning in canvas-status-hud-layer.ts) + 6 regression tests. VERIFIED clean: header reads `PF 41/63 CA 18 LV 10`. 3306 tests pass. Debug session: `.planning/debug/resolved/canvas-body-blank-ws-drop.md`. See memory [[g2app-hud-launch-chain-status]].
Resume file: None
Resume cmd: HUD fix COMMITTED `748d304` (fix(g2-app): CHROME-01 + FIX-DD-01 + changeset, biome-clean, 23 status-hud tests green) on branch feat/hud-raster-rendering. Clean screenshot emailed (msg 19eaaab2588c). REMAINING: tidy the still-dirty tree (EVF-19 deletions + quick-260604 planning docs to commit + gitignore _seed.ts/release-artifacts/foundry-module/release). Services bridge :8911 / vite :5173 / sim :9898 still running (stop if not needed).
