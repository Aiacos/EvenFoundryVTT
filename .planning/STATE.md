...
Last activity: 2026-06-05 -- Completed quick tasks 260605-etr + 260605-f9s: glasses HUD is now FULLY CLEAN in the sim (header/footer/map-capture/z05 all render real or spec content, ZERO SDK "Text" placeholders; status-hud shows Artemis · PF 55/88 · CA 18). Render-polish DONE + live-verified. NEXT: (2) character selection — wire characterId end-to-end (bridge on-connect push uses characters[0]; wizard step3 saves characterId to Tier3 but it never reaches launch→bootEngine→handshake→bridge); (3) real release — .ehpk pack + Docker Compose self-host guide in docs/README + version bumps. Temp live setup running: local bridge :8911 (dev-no-auth, Artemis seeded), vite repointed to :8911, sim :9898 — restart vite with remote bridge URL to restore user's original setup.
