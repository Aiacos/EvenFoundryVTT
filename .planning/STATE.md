...
Last activity: 2026-06-05 -- Completed quick task 260605-etr (production-clean HUD: header+footer render real content, no SDK "Text"; live-sim verified). Sibling bug remains (z05 idle-infill + map-capture still SDK "Text" — idleInfill.draw() not called post-bundle) → fixing next, then character selection (260605-d0v uses characters[0]; wire characterId end-to-end) + real release (.ehpk pack + docker compose self-host guide in docs/README). Temp live setup running: local bridge :8911 (dev-no-auth) with Artemis seeded, vite repointed to :8911, sim on :9898 — restart vite with the remote bridge URL to restore the user's original setup.
