...
Last activity: 2026-06-05 -- Completed quick task 260605-e9t + LIVE SIM VERIFIED end-to-end: g2-app wsEvents now uses a persistent listener + last-value replay (bus created pre-handshake), so the on-connect character.delta is no longer dropped. With d0v (on-connect push) + dog (snapshot cache) + e9t (replay), the glasses HUD renders REAL character data on FIRST connect in the EvenHub simulator (Artemis · PF 55/88 · CA 18 from the seeded cache, no post-connect delta). 1429/1429 g2-app tests. Temp live setup still running: local bridge :8911 (dev-no-auth), vite repointed to :8911 — restart vite with the remote bridge URL to restore.
