...
Last activity: 2026-06-05 -- Completed quick task 260605-flv (character selection end-to-end) + live-sim verified: HUD now renders the CHOSEN PC (loaded ?actor=Dante → "Dante Lanzu… PF 41/63", not Artemis). Render-polish (etr+f9s) + on-connect data (d0v+dog+e9t) + selection (flv) all DONE and live-verified. NEXT (final task 3): real release — version bumps + changesets, build + pack g2-app .ehpk (Even Hub portal submit is MANUAL — prepare steps), Docker Compose self-host guide for other users in docs/ + README section. Even Hub deploy infra already exists (deploy/bridge.Dockerfile, docker-compose.yml + .https.yml + Caddyfile + .env.example, docs/release/*, CI evenhub-pack.yml); app.json whitelist is placeholder (per-user origin). Temp live setup running: bridge :8911 (dev-no-auth, FLV code, Artemis+Dante seeded), vite repointed to :8911, sim :9898 — restart vite with remote bridge URL to restore user's original setup. NOTE: when restarting the dev bridge, re-seed roster+snapshots (in-memory cache).
