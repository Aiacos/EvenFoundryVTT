...
- **Contratti qualità:** nuovo contratto **INV-1 raster** (snapshot deterministico dell'output canvas / hash dei tile, sostituisce le fixture text per la HUD) + coerenza **INV-3** (Specs §7 raster-HUD layout, README, `docs/showcase/index.html` nello stesso commit).

**Key context:** Decisioni locked dall'utente 2026-06-05 (vedi `.planning/TODO-hud-raster.md`, ADR-0013, memory `raster-ui-milestone-direction`). Compositing su canvas unico confermato (forzato dai 4 image-container). Render = mix statico+dinamico. Feats/Bio: estendere subito schema+reader. ~90% della content-logic esiste già (6 tab renderer in `character-sheet-tab-renderers.ts`, `combat-tracker-panel.ts`, `LayerManager`/`layer-types.ts`, pipeline raster in `src/hud/*` + `map-base-layer.ts`, R1 gesture bus) — il lavoro è uno swap di substrato verso canvas + un compositor canvas. xxhash sub-tile delta per ~5fps resta TODO-hud-raster #2 (fuori scope). Branch di lavoro: `feat/hud-raster-rendering`. **v0.9.14 Release & Distribution resta PARCHEGGIATO** (era stato avviato come Phase 19 poi sospeso per il pivot raster; sarà ripreso in un milestone successivo).
...
1. **xxhash ~5fps delta loop (TODO-hud-raster #2)** — sub-tile delta diffing nel RasterController così solo i tile CHANGED vengono ri-encodati/spediti (idle ≈ banda zero).
