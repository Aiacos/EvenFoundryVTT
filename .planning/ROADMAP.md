...
**Milestone Goal:** Sostituire il substrato di rendering della UI HUD da text-container 27px a immagini raster compositate su un singolo canvas 576×288 → dither → 4 tile → push ai 4 image-container SDK, con full controllo tipografico, scheda PG a 6 tab, combat tracker, delta loop ~5fps e promozione a boot di default.

- [ ] **Phase 19: ADR-0013 Amendment 1 + INV-2 Re-verify + Canvas Compositor Core** — ADR scritto prima dell'impl · tile 288×144 vs 200×100 doc-cap verificato · `CanvasCompositor` + `CanvasLayer` + `buildHudRasterPageSchema()` (5 container: 4 tile + 1 capture) · `LayerManager` `renderMode` + `_flushPage()` mode-selector + `_compositeAndPush()` + fixed-budget assertion · glyph path byte-identico; NESSUNA UI change (RAST-01..05, RINV-02)
...
- [ ] **Phase 24: Delta Loop ~5fps xxhash** — loop `~5fps` + debounce 200ms + `TileDelta` sub-tile xxhash con geometria 288×144 · solo tile CHANGED vengono re-encodati/spediti · HUD idle ≈ banda BLE quasi-zero (RPROMO-01)
...
**Success Criteria** (what must be TRUE):
  1. ADR-0013 Amendment 1 è presente su disco e ratificato, descrivendo: compositor model Option B (per-layer OffscreenCanvas + drawImage), capture-container re-mapping (5° text container zero-size `hud-capture`), budget-mode canvas (fixed 5 container, `getContainerCount()` returns `{image:0, text:0}` per canvas layer), `renderMode: 'canvas' | 'glyph'`, selettore `_flushPage()` — scritto PRIMA di qualunque merge dell'implementazione
  2. Un documento INV-2 re-verify (o entry nel changelog ADR-0013 Amendment 1) cita il risultato del fetch su `hub.evenrealities.com/docs/guides/device-apis` del 2026-06-05 o successivo, registrando se 288×144 è confermato, non documentato, o confutato; la costante `HUD_TILE_GEOMETRY` è parametrizzata con un flag config che consente switch tra `FULL_SCREEN_2x2` (288×144) e `DOCUMENTED_LIMIT_2x2` (200×100) [SC hardware reale: `human_needed` per ADR-0005 Branch A]
...
**Success Criteria** (what must be TRUE):
  1. Un'istanza `TileDelta` con geometria 288×144 (NON i 200×100 della map) è usata per hashing sub-tile; solo i tile con hash cambiato vengono re-encodati e spediti via `pushHudTiles`; in HUD idle (nessun `character.delta` né `combat.delta`) zero tile vengono respinti dopo il primo frame
