...
---
*Last reorganized: 2026-05-18 — **v0.9.13 ARCHIVED**. Lifecycle complete: audit ✅ (9/9 REQ-IDs satisfied) → complete-milestone ✅ (archive `milestones/v0.9.13-{ROADMAP,REQUIREMENTS}.md` + tag `v0.9.13` annotated) → cleanup ✅ (3 phase dirs moved to `milestones/v0.9.13-phases/`). Workspace tests 2668/2668; CI Gate 8 socketlib count = **17** preserved. 9/9 v1 REQ-IDs Resolved (3 SHEET ability + 3 SHEET skills + 3 INFILL-14.1 sub-items). Software-only. Next: `/gsd-new-milestone` to open v0.9.14 — likely candidate is Skill Check + Saving Throw write path (`skill_check` tool is currently stub in `bridge/src/routes/tools-dispatch.ts:80`; gesture wiring on Skills tab scroll-tap currently inert; would bump CI Gate 8 17 → 18).*
