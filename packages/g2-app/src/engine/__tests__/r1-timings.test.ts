 ...
 *   - RT-03: longPressMs >= 500 (Specs §3.2 / §10.0.1 GO criterion regression guard)
 *
...
describe('DEFAULT_R1_TIMINGS (RT-01..RT-03)', () => {
  it('RT-01: has locked timing values (tapMs=250, doubleTapWindowMs=350, longPressMs=600, scrollDebounceMs=50)', () => {
    expect(DEFAULT_R1_TIMINGS.tapMs).toBe(250);
    expect(DEFAULT_R1_TIMINGS.doubleTapWindowMs).toBe(350);
    expect(DEFAULT_R1_TIMINGS.longPressMs).toBe(600);
  ...
  it('RT-03: longPressMs >= 500 (Specs §3.2 / §10.0.1 GO criterion ≥500 ms)', () => {
    expect(DEFAULT_R1_TIMINGS.longPressMs).toBeGreaterThanOrEqual(500);
  });
});
