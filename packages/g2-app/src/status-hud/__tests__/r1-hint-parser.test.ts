 ...
 *
 * Tests (RHP-*):
 *   - RHP-01: happy path — standard format `tap=X scroll=Y qa=Z` parsed correctly
 *   - RHP-02: order-independent — `scroll=Y tap=X qa=Z` parsed correctly
 ...
 *   - RHP-07: `q[sheet]` bracket format preserved verbatim (quickActionLabel includes brackets)
 *   - RHP-08: DE locale string parsed correctly (e.g. `scroll=Init tap=Schnell qa=q[Kampf]`)
 *
...
describe('parseR1HintString', () => {
  it('RHP-01: parses canonical format tap=X scroll=Y qa=Z', () => {
    const result = parseR1HintString('tap=cycle  scroll=nav  qa=quick');
    expect(result.tap).toBe('cycle');
    expect(result.scroll).toBe('nav');
    expect(result.quickActionLabel).toBe('quick');
  });

  it('RHP-02: order-independent — scroll=Y tap=X qa=Z parsed correctly', () => {
    const result = parseR1HintString('scroll=iniz tap=rapida qa=q[combat]');
    expect(result.tap).toBe('rapida');
    expect(result.scroll).toBe('iniz');
    expect(result.quickActionLabel).toBe('q[combat]');
  });
  ...
  it('RHP-03: missing field defaults to empty string', () => {
    ...
    expect(result.tap).toBe('cycle');
    expect(result.scroll).toBe('nav');
    expect(result.quickActionLabel).toBe('');
  });
  ...
  it('RHP-04: extra whitespace between tokens is tolerated', () => {
    ...
    const result = parseR1HintString('tap=cycle-tab  scroll=cont  qa=q[sheet]');
    expect(result.tap).toBe('cycle-tab');
    expect(result.scroll).toBe('cont');
    expect(result.quickActionLabel).toBe('q[sheet]');
  });
  ...
  it('RHP-05: malformed input (empty string) returns three empty strings without throw', () => {
    ...
    expect(result).toEqual({ tap: '', scroll: '', quickActionLabel: '' });
  });
  ...
  it('RHP-07: q[sheet] bracket format preserved verbatim in quickActionLabel', () => {
    const result = parseR1HintString('tap=cambia-tab scroll=cont qa=q[sheet]');
    expect(result.quickActionLabel).toBe('q[sheet]');
    // Full bracket preserved — not truncated or modified.
    expect(result.quickActionLabel).toContain('[');
    expect(result.quickActionLabel).toContain(']');
  });

  it('RHP-08: DE locale string parsed correctly', () => {
    const result = parseR1HintString('scroll=Init tap=Schnell qa=q[Kampf]');
    expect(result.tap).toBe('Schnell');
    expect(result.scroll).toBe('Init');
    expect(result.quickActionLabel).toBe('q[Kampf]');
  });
});
