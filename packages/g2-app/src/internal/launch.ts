 ...
 *
 *   - **Branch A — no-auth dev:** when `isNoAuth()` is `true`, boot the canvas
 *     engine immediately via `bootEngine({ bridgeUrl, token, locale })`.
...
export interface LaunchDeps {
   ...
   *
   * Used to extract the `?actor=<id>` URL param in the no-auth dev branch
   * (sim / quick testing).
   */
