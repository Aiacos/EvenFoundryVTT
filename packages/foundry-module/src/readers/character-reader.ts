...
/**
 * Stable deterministic fallback id for items that lack `item.id`.
 *
 * A `Math.random()` fallback gave the same id-less item a *different* id on
 * every snapshot, defeating the g2-app's downstream diff/dedup (it keys on id)
 * and producing spurious churn. This derives an id from the item's name and its
 * positional index via a small FNV-1a hash, so the same id-less item in the same
 * slot yields the same id across snapshots while distinct items stay distinct.
 *
 * The `evf-` prefix namespaces the synthetic id so it can never collide with a
 * real Foundry document id (16-char base62).
 *
 * @param name  - The item name (may be `undefined`/empty — folded into the hash).
 * @param index - The item's positional index within its source collection.
 * @returns A stable `evf-<hex>` id string.
 * @internal
 */
function stableFallbackId(name: string | undefined, index: number): string {
  const seed = `${name ?? ''}\u0000${index}`;
  // FNV-1a 32-bit — deterministic, dependency-free, sufficient for id derivation.
  let hash = 0x811c9dc5;
  for (let i = 0; i < seed.length; i++) {
    hash ^= seed.charCodeAt(i);
    hash = Math.imul(hash, 0x01000193);
  }
  return `evf-${(hash >>> 0).toString(16).padStart(8, '0')}`;
}
...
function extractInventory(actor: ReturnType<typeof game.actors.get>): InventoryItem[] {
  ...
  for (const [index, raw] of contents.entries()) {
    ...
    const entry: InventoryItem = {
      id:
        (item.id as string | undefined) ?? stableFallbackId(item.name as string | undefined, index),
...
function extractSpellbook(actor: ReturnType<typeof game.actors.get>): Spellbook {
  ...
  for (const [index, raw] of contents.entries()) {
    ...
    // dnd5e 5.1 renamed SpellData#preparation.mode → method and .prepared → prepared.
 ...
 * the other readers in this file.
 *
 ...
 *
 * Never throws on missing structure — mirrors null-safety of {@link extractFeats}.
...
export function getCharacterSnapshot(actorId: string): CharacterSnapshot | null {
  ...
  // actor.img is a non-empty string. The G2 app fetches it same-origin (T-13-02).
  ...
  return {
    ...
    hp: hp.value,
    maxHp: hp.max,
    // dnd5e leaves hp.temp null (not 0) when there is no temp HP; coerce to 0 so
    // the snapshot passes CharacterSnapshotSchema (tempHp: number().nonnegative()).
    // Without this the schema rejects — and silently drops — the whole snapshot for
    // any actor with no temporary HP.
    tempHp: hp.temp ?? 0,
    ac: ac.value,
