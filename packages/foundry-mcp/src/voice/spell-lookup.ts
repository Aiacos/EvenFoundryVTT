...
export const SPELL_LOOKUP: ReadonlyArray<SpellLookupEntry> = Object.freeze([
  ...
  { dnd5eId: 'lesser-restoration', it: 'restaurazione inferiore', en: 'lesser restoration', level: 2 },
...
/**
 * Resolve a voice transcript fragment to a dnd5e spell ID.
 *
 * Resolution order:
 * 1. Exact EN match → 'en-table'
 * 2. Exact IT match → 'it-table'
 * 3. Levenshtein ≤ 2 across both columns, per-entry minimum distance:
 *    - Single winner → 'fuzzy' with distance
 *    - Multiple entries tied at minimum distance → 'ambiguous' with candidates
 * 4. No match → 'none'
 *
 * T-12-03: returned dnd5eId is ALWAYS sourced from SPELL_LOOKUP — never
 * constructed from the caller's input string.
 *
 * @param transcript - Raw or pre-normalised transcript fragment. Empty → 'none'.
...
export function lookupSpellId(transcript: string): SpellLookupResult {
  ...
  // 3. Levenshtein — compute min distance per entry across both columns
  ...
  if (candidates.length === 1) {
    ...
    // T-12-03 guard: winner is guaranteed to exist (length === 1)
