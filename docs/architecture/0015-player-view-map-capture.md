...
- **`streaming`** — joins as the Foundry user chosen from the **app's character selector**: the bridge resolves the selected PC's `actorId` → its owning Foundry user (opt-in gated, like `actor`) and the headless selects that user by `/join` label (blank password). When no PC is selected or its owner has not opted in, the headless falls back to the **configured env stream user** `EVF_PLAYER_VIEW_STREAM_USER` (a GM/observer whose viewport frames the party), and finally to the **first** `/join` option (non-deterministic). The app value wins over the env. ⚠️ Joining as a PC's user shows THAT user's fogged/lit view (like `actor`) — for a full-party stream, select a GM/observer PC or configure the env user.
...
> ⚠️ **Auto-entry guard:** if Foundry auto-enters the world (a live session cookie) the `/join` `<select>` is absent and neither the stream user nor the actor user gets applied — you'd silently get the storageState's baked-in user. The launcher therefore **asserts** `game.user.name` matches the requested user after `canvas.ready`; a mismatch fails the launch (`error`/`unavailable`) rather than streaming the wrong view. (The assertion is skipped only for `streaming` with no configured stream user, where any user is acceptable by definition.)
