...
> **Status note (v0.13.0, 2026-09-25)** — `validate:direct-sideload` is retired with the Foundry-served page ([ADR-0019](./0019-relay-pairing-player-projector.md)); the direct-model GO/NO-GO is now `pnpm --filter @evf/validation-harness validate:relay` (relay health, CORS, room round-trip + the G1–G2 hardware checklist). The 2×2 tile-geometry gate is unchanged.
