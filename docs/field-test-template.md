...
(`docs/field-test-N.md`) along with the `validate:direct-sideload` evidence JSON of the same
day (see [runbook](runbook.md) §Sideload GO/NO-GO harness).
...
| Even Hub SDK version | `@evenrealities/even_hub_sdk@0.0.15` (or _pending_ if updated) |
| Even Realities App version | _pending_ (≥ 2.2.9 for long-press) |
| Foundry / dnd5e / module version | _pending_ |
| Environment | _pending_ (clean / 5 GHz-loaded / 2.4 GHz+microwave — see Specs.md §10.0.3) |
| Foundry HTTPS exposure | _pending_ (reverse proxy + Let's Encrypt / Tailscale / native TLS) |
...
- [ ] *no GM connected* / *credentials rejected* on the phone page — count: _pending_
...
- [`docs/runbook.md`](runbook.md) — phone-page diagnostics and the `validate:direct-sideload` harness.
