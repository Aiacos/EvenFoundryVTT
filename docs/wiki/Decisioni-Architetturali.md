...
|---|---|---|
| [ADR-0019](https://github.com/Aiacos/EvenFoundryVTT/blob/develop/docs/architecture/0019-relay-pairing-player-projector.md) | Relay e collegamento dal Foundry del giocatore: la scheda che mostra il QR è il projector, il telefono non accede mai a Foundry; relay opaco a stanze (Cloudflare Worker + Durable Object) su un indirizzo fisso; app unica come `.ehpk` / GitHub Pages / Vite; asset della mappa preparati dal projector | accettato |
...
| [ADR-0011](https://github.com/Aiacos/EvenFoundryVTT/blob/develop/docs/architecture/0011-foundry-write-path-single-workflow-origin.md) | Write path con origine unica: ogni mutazione passa da `dispatchTool`; solo `src/write-path/` chiama `activity.use()` (gate CI 8) | accettato (l'origine è l'unica scheda che trasmette quel dispositivo, ADR-0019; l'emendamento 2 «esecuzione del giocatore via poller» è superato) |
...
| [ADR-0017](https://github.com/Aiacos/EvenFoundryVTT/blob/develop/docs/architecture/0017-player-owned-glasses-hybrid-projector.md) | Occhiali dei giocatori: abilitazione una tantum del GM, associazione self-service dal Foundry del giocatore, chiavi ECDH P-256, projector ibrido (client del giocatore se online, altrimenti un GM) | superato da 0019 (niente più abilitazione del GM, utenti «(G2)», chiavi ECDH né elezione) |
| [ADR-0014](https://github.com/Aiacos/EvenFoundryVTT/blob/develop/docs/architecture/0014-bearer-actor-authorization.md) | Bearer legato all'utente Foundry + autorizzazione per attore | superato da 0017, a sua volta superato da 0019 (il controllo di proprietà per attore resta) |
