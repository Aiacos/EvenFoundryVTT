...
- **App per gli occhiali** = un solo bundle (`packages/g2-app/dist`) in tre modi: **pacchetto Even Hub** «FoundryVTT G2 HUD» (giocatori: gruppo beta, poi store; sopravvive al blocco del telefono; «Scansiona QR» con la fotocamera del telefono o «Inserisci codice»), **GitHub Pages** `/app/` (la pagina che il QR apre con Even Realities App › Scan QR in modalità sviluppatore), **Vite dev** in LAN (`pnpm dev:glasses`). `app.json`: SDK ≥ 0.0.16, whitelist `https://` + `wss://evf-relay.aiacos.workers.dev`, permesso `camera`. Informativa privacy: `docs/privacy.md`.
...
> **v0.13.0 ([ADR-0019](docs/architecture/0019-relay-pairing-player-projector.md)):** l'unica origine in whitelist è il relay del progetto (`https://` + `wss://evf-relay.aiacos.workers.dev`, le due grafie finché il gate G2 non conferma quale serve); il relay risponde con CORS e l'app non contatta mai l'origine di Foundry. HTTPS resta obbligatorio per l'app e il relay; **Foundry non ha più bisogno di HTTPS pubblico**. Re-verified ✓ 2026-09-25: whitelist per origine completa, niente wildcard, WebSocket soggetti alla stessa whitelist (*«Expect drops when the WebView backgrounds»*, `reference/faq`), whitelist non è un bypass CORS (`build/networking`).
...
**Substrato di rendering predefinito — Showcase raster HUD (v0.11.0):**

Dal milestone **v0.11.0** il **substrato default** non è più `hybrid` ma il nuovo mode **`'showcase'`**: l'**intero HUD glanceable** è renderizzato come **una singola immagine raster 400×200 4-bit** (4 image tile × 200×100 px — il tetto hardware §7.4b.3), centrata sullo schermo 576×288. Un unico frame `ShowcaseHudLayer` (`engine/hud-render-mode.ts` → `HudRenderMode = 'canvas' | 'glyph' | 'hybrid' | 'showcase'`, default risolto in `boot-engine-core`) disegna, fedele a `docs/showcase/index.html` (font pixel VT323 + palette phosphor-green):

- cornice D&D a **doppio filetto** + bracket d'angolo;
- **header** (scena · round/turn · battery R1);
- **regione MAP** incorniciata;
- **card status D&D** a destra (nome, livello+classe, barra `♥` PF + numerico, `⛨` CA, **VELOCITÀ con icona STIVALE** — NON spade, `⚔` era errato per il movimento — pips slot incantesimo `▓░`, condizioni `▶`);
- **footer** con gli hint gesture R1 canonici (scroll / tap / `▲`=menu — nessun long-press, §GEST-01).

Gli **overlay z=2** (menu Quick Action, scheda PG, combat tracker, spellbook, inventario, target-picker) funzionano in showcase mode via un meccanismo generico: i pannelli canvas esistenti compositano a 576×288 e vengono **downscalati uniformemente** (0.6944×, stesso aspect 2:1, nessuna distorsione) dentro la regione raster 400×200. `canvas` / `hybrid` / `glyph` restano **fallback selezionabili** via override kv `view.hud.render`; il degrade BLE-degraded forza sempre `glyph`. L'icona velocità è ora un **vettore STIVALE** nei path canvas (`panels/icon-dictionary.ts` → `drawIcon(IconId.Speed)`; `⚔` resta solo come text-fallback low-bandwidth). **Perf:** il loop delta HUD calcola l'hash dei **source tile pre-encode** (`engine/hud-delta-driver.ts` `_hashSourceTile`), così i tile invariati non vengono mai ri-dithered/ri-encodati (idle → zero encode).
...
> **Contesto v0.11.0:** questo è il percorso di rendering di **fallback BLE-degraded** (o path glyph esplicito via `view.hud.render = "glyph"` / `view.map.mode = "glyph"`). Il substrato di rendering **default** dal milestone **v0.11.0** è il **showcase raster HUD** (whole-HUD 400×200, mode `'showcase'`) descritto in §7.2; `canvas`/`hybrid` restano fallback selezionabili. Il mockup qui sotto è il contratto INV-1 per il path text-container SDK; NON va cancellato — è la spec del fallback glyph e rimane il contratto di snapshot per la glyph suite di `inv:all`.
...
║                              [ ✓ ] G2 display 576×288                                     ║
║                              [ ✓ ] R1 ring paired (92%)                                   ║
║                              [ ⟳ ] Relay wss://evf-relay.aiacos.workers.dev               ║
...
- **2026-07-04 (v0.11.0 — milestone: substrato default = Showcase raster HUD whole-HUD 400×200)** — Il substrato di rendering **default** degli occhiali cambia da `hybrid` al nuovo mode **`'showcase'`**, fedele a `docs/showcase/index.html`. **Bump v0.10.0 → v0.11.0.** Doc-only sweep (INV-3 atomic: Specs §7.2/§7.4 + README badge/bullet + showcase version/milestone). Sintesi del feature shipped (verificato sul codice):
  ...
  - **Overlay via downscale:** gli overlay z=2 (Quick-Action menu, scheda PG, combat tracker, spellbook, inventario, target-picker) funzionano in showcase mode via meccanismo generico — i pannelli canvas compositano a 576×288 e vengono downscalati uniformemente (**0.6944×**, stesso aspect 2:1, no distorsione) nella regione raster 400×200.
  - **Fallback selezionabili:** `canvas` / `hybrid` / `glyph` restano selezionabili via override kv `view.hud.render` (`engine/hud-render-mode.ts` → `HudRenderMode = 'canvas' | 'glyph' | 'hybrid' | 'showcase'`); il degrade BLE-degraded forza sempre `glyph`.
  - **Perf:** il loop delta HUD calcola l'hash dei **source tile pre-encode** (`engine/hud-delta-driver.ts` `_hashSourceTile`), così i tile invariati non vengono mai ri-dithered/ri-encodati (idle → zero encode).
  - **Test:** suite `@evf/g2-app` **1964 verdi**.
  - **Re-verified ✓ / no new upstream claims — INV-2 unchanged** (bump di architettura di rendering interna: nessun nuovo claim hardware/upstream — G2 576×288 4-bit · 4 image tile ≤400×200 · execution model phone WebView · gesture set · no speaker/no camera già verificati e invariati; nessun WebFetch nuovo richiesto).
  - **§7.2:** paragrafo "Substrato di rendering predefinito — Showcase raster HUD (v0.11.0)" aggiunto; §7.4 nota "Glyph Fallback Mode" aggiornata; mockup INV-1 (glyph 27px + boot splash) invariati per allineamento.
