...
**EvenFoundryVTT streams your D&D character straight from Foundry to the G2 glasses** — no server to install, no URL to type: the GM shows a QR code in Foundry, you scan it with the Even Realities App, and your **character sheet** (⅓ of the display) and a **pixelated tactical map** (⅓) stay in your field of view while the last third follows the action.
...
```
┌──────────── ⅓ ────────────┬──────────── ⅓ ────────────┬──────────── ⅓ ────────────┐
│ CHARACTER SHEET           │ PIXEL MAP (192×288)       │ CONTEXT                   │
│ HP · AC · abilities       │ follows your token        │ log · initiative ·        │
│ action economy · slots    │ walls · light · tokens    │ actions · targets ·       │
│ conditions · 4 pages      │ ≤ 1 fps, tile deltas      │ spells · results          │
└───────────────────────────┴───────────────────────────┴───────────────────────────┘
```

Eleven glasses mocks (exploration, combat, action menu, target picker, spells, roll result, reaction, skills page, first run, connecting, offline) plus three phone/Foundry mocks live in **[`docs/design/g2-thirds-layout.md`](docs/design/g2-thirds-layout.md)** — they are the INV-1 layout contract.
...
- **Always-on character sheet** — 4 pages (Main, Combat, Skills & Saves, Spells/Inventory), auto-switch to Combat when initiative starts.
- **Pixelated map** — built on the phone from compact scene data (grid, walls, lights, visible tokens, background), 4-bit greyscale, 2 × 192×144 tiles, only changed tiles are sent, paced to the SDK's 100 ms image limit.
...
**v0.10.0 — direct streaming.** The Node bridge, the `foundry-mcp` server and Docker Compose were removed (ADR-0012); the G2 app was redesigned around the thirds layout on Even Hub SDK 0.0.15. Hardware verification (QR sideload, cookie persistence, BLE map pacing) follows the defer-hardware pattern: `pnpm --filter @evf/validation-harness validate:direct-sideload`.
...
| **v0.10.0** | 🚧 current | direct Foundry → G2 streaming, thirds HUD, one-scan pairing |
...
- **Shared**: `@evf/shared-protocol` (Zod schemas + WebCrypto sealed envelope), `@evf/shared-render` (INV-1 grid matcher)
...
- [`docs/design/g2-thirds-layout.md`](docs/design/g2-thirds-layout.md) — thirds layout, input model, pairing flow, 14 mocks
